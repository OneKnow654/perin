...
const inputCls =
  "w-full bg-gray-50 border border-surface-divider rounded-xl px-5 py-3.5 text-primary placeholder:text-text-muted focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/5 focus:outline-none transition-all duration-300";
      ...
      <section className="py-16 lg:py-24">
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
            ...
            <motion.div
              className="bg-white rounded-[2rem] border border-surface-divider p-8 lg:p-10 shadow-xl shadow-primary/5 order-2 lg:order-1 h-fit"
              variants={fadeLeft}
              initial="hidden"
              whileInView="visible"
              viewport={viewport}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-xs font-bold uppercase tracking-widest text-primary">Phone Number</label>
                      <span className="text-[10px] text-text-muted font-bold">{form.phone.length}/10</span>
                    </div>
                    <input
                      type="tel"
                      name="phone"
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-xs font-bold uppercase tracking-widest text-primary">Message *</label>
                      <span className={`text-[10px] font-bold ${form.message.length >= 50 ? 'text-red-500' : 'text-text-muted'}`}>
                  ...
                  <motion.button
                    type="submit"
                    className="w-full inline-flex items-center justify-center gap-2 text-white bg-primary px-10 py-4 rounded-2xl font-bold transition-all shadow-xl shadow-primary/20"
            ...
            <div className="space-y-12 order-1 lg:order-2">
                ...
                >
                  {infoCards.map(({ Icon, title, value, href }, i) => (
                    <motion.a
                      ...
                      href={href}
                      variants={fadeUp}
                      className="block bg-white rounded-2xl border border-surface-divider p-5 hover:border-primary hover:shadow-lg transition-all duration-300 group"
                      whileHover={{ y: -3 }}
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-primary/5 flex items-center justify-center flex-shrink-0 group-hover:bg-primary transition-colors">
                          <Icon size={16} className="text-primary group-hover:text-white transition-colors" />
                        </div>
                        <div>
                          <h4 className="font-bold text-primary text-xs uppercase tracking-wider mb-0.5">{title}</h4>
                          <p className="text-text-muted text-sm font-medium">{value}</p>
                        </div>
                      </div>
                    </motion.a>
                  ))}
              ...
              {/* Map Integrated Here */}
              <motion.div
                className="w-full h-[330px] rounded-3xl overflow-hidden border border-surface-divider shadow-xl relative group"
                variants={fadeUp}
                initial="hidden"
                whileInView="visible"
                viewport={viewport}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <motion.div
                  ...
                  whileInView="visible"
                  viewport={viewport}
                  className="rounded-3xl p-6 border border-surface-divider bg-gray-50"
                >
                  <h4 className="font-bold text-primary text-sm mb-4 uppercase tracking-widest">Follow Us</h4>
                ...
                <motion.div
                  ...
                  whileInView="visible"
                  viewport={viewport}
                  className="rounded-3xl p-6 bg-primary text-white"
                >
